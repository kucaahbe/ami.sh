...
show_and_run "sudo debootstrap --arch $AMIARCH --include=ssh,curl,linux-image-xen-amd64 squeeze $AMIROOT $AMI_REPO_PROTOCOL://$AMI_REPO_URL"
...
echo "	kernel /boot/vmlinuz-2.6.32-5-xen-amd64 root=/dev/xvda1" | sudo tee -a $AMIROOT/boot/grub/menu.lst
echo "	initrd /boot/initrd.img-2.6.32-5-xen-amd64"              | sudo tee -a $AMIROOT/boot/grub/menu.lst
